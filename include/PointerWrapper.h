...
public:
     ...
     */
    /*
     * Here we can't call swap because this.ptr could initialized with a random value (not owned by our process) 
     * thus when other goes out of scope and the destructor is called it might crash the program (SEGFAULT) and cause undefined behaviour 
    */ 
    ...
    T& operator*() const {
        ...
        if(ptr == nullptr) {
            throw std::runtime_error("Attempting to dereference a null PointerWrapper!!! Crash now!!!");
        }
    ...
    T* operator->() const {
        ...
        if(ptr == nullptr) {
            throw std::runtime_error("Attempting to call -> operator on a null PointerWrapper!!! Crash now!!!");
        }
    ...
    T* get() const {
        if(ptr == nullptr) {
            throw std::runtime_error("Attempting to get() on a null PointerWrapper!!! Crash now!!!");
        }
